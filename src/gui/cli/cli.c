...
void print_start_screen(WINDOW *win, int highscore) {
  ...
  int start_y = WINDOW_HEIGHT / 2 - message_count;
  print_rectangle(win, start_x, start_y, message_length + 2, 2 * message_count);
  mvwprintw(win, start_y + 1, start_x + 1, "Press key 0-9 to start the level");
  mvwprintw(win, start_y + 3, start_x + 1 + message_length / 4, "Highscore: %d",
            highscore);
