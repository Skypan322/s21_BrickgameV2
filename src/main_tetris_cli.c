...
void game_loop(WINDOW *win, Tetris *tetris) {
    ...
    if (tetris->state == GAMEOVER) {
        tetris->highscore = tetris->score > tetris->highscore
                                ? tetris->score
                                : tetris->highscore;
...
int main() {
    ...
    box(win, 0, 0);
    long highscore = read_highscore("highscore_tetris.txt");
    ...
    tetris.highscore = read_highscore("highscore_tetris.txt");
    game_loop(win, &tetris);
    write_highscore("highscore_tetris.txt", tetris.highscore);
    getch();
    endwin();
    return 0;
}